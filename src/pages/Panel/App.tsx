...
function App() {
  ...
  function handleAccordionChange(hitKey: number) {
    ...
    newHitList.set(hitKey, {
      ...newHitList.get(hitKey),
      expanded: !newHitList.get(hitKey)!.expanded,
    });
  ...
  useEffect(() => {
    ...
    function addNewHit(
      ...
      setHitList(oldHitList => {
        ...
        let hitListKey =
          oldHitList.size > 0 ? Math.max(...oldHitList.keys()) + 1 : 0;

        // Append new hit on hitList
        return new Map(
          oldHitList.set(hitListKey, {
