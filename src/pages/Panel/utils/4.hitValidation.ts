 ...
 */
function penguinValidation(messageData: Object): Promise<Array<ValidationResultInterface>> {
    return new Promise((res, rej) => {
        ...
        (document?.getElementById('theFrame') as HTMLIFrameElement).contentWindow?.postMessage(messageData, '*', [channel.port2]);
    });
}
 ...
 */
async function validateHit(dataLayerSchema: { [key: string]: any }, requiredParametersToCheck: string[], parametersObj: { [key: string]: string }) {
        ...
        const missingRequiredParameters =
            requiredParametersToCheck
                .map(function (element) {
                    return RW.identifyUndefinedRequiredParameters({ type: element, params: parametersObj });
                })
